...
: ${BATCH_SIZE:=48}
: ${GRAD_ACCUMULATION:=1}
: ${OUTPUT_DIR:="/disk/scratch3/jomahony/interspeech2023_models/baseline_larger_batch"}
: ${DATASET_PATH:=None}
: ${TRAIN_FILELIST:=filelists/interspeech_filelists/interspeech_baseline_train.txt}
: ${VAL_FILELIST:=filelists/interspeech_filelists/interspeech_baseline_val.txt}
: ${AMP:=true}
: ${SEED:=""}
...
: ${PROJECT:=baseline-interspeech_amp}
: ${EXPERIMENT_DESC:=baseline model for interspeech}

# Corpus specific mean and std across training data
: ${CORPUSMEAN:=160.39}
: ${CORPUSSTD:=54.61}
